 ...
 * @api private
 */
NIFCLOUD.ResourceWaiter = inherit({
  /**
   ...
   *   var ec2 = new NIFCLOUD.EC2;
   *   var waiter = new NIFCLOUD.ResourceWaiter(ec2, 'instanceRunning');
   */
  constructor: function constructor(service, state) {
    ...
    if (typeof this.state === 'object') {
      NIFCLOUD.util.each.call(this, this.state, function (key, value) {
        this.state = key;
  ...
  Listeners: {
    retry: new NIFCLOUD.SequentialExecutor().addNamedListeners(function(add) {
    ...
    output: new NIFCLOUD.SequentialExecutor().addNamedListeners(function(add) {
    ...
    error: new NIFCLOUD.SequentialExecutor().addNamedListeners(function(add) {
  ...
  setError: function setError(resp, retryable) {
    resp.data = null;
    resp.error = NIFCLOUD.util.error(resp.error || new Error(), {
  ...
  checkSuccess: function checkSuccess(resp) {
    ...
    var r = NIFCLOUD.util.jamespath.find(this.config.successPath, resp.data);
  ...
  loadWaiterConfig: function loadWaiterConfig(state, noException) {
    if (!this.service.api.waiters[state]) {
      ...
      throw new NIFCLOUD.util.error(new Error(), {
