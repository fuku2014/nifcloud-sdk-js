 ...
 * @api private
 */
NIFCLOUD.ParamValidator = NIFCLOUD.util.inherit({
    ...
    if (this.errors.length > 1) {
      ...
      if (this.errors.length > 1) {
        msg = 'There were ' + this.errors.length +
              ' validation errors:\n* ' + msg;
        throw NIFCLOUD.util.error(new Error(msg),
  ...
  fail: function fail(code, message) {
    this.errors.push(NIFCLOUD.util.error(new Error(message), {code: code}));
  },
  ...
  validateType: function validateType(context, value, acceptedTypes, type) {
    ...
    for (var i = 0; i < acceptedTypes.length; i++) {
      if (typeof acceptedTypes[i] === 'string') {
        ...
        if (NIFCLOUD.util.isType(value, acceptedTypes[i])) return;
        if (!type && !foundInvalidType) acceptedTypes = acceptedTypes.slice();
        acceptedTypes[i] = NIFCLOUD.util.typeName(acceptedTypes[i]);
      }
  ...
  validatePayload: function validatePayload(context, value) {
    ...
    if (NIFCLOUD.util.isNode()) { // special check for buffer/stream in Node.js
      var Stream = NIFCLOUD.util.nodeRequire('stream').Stream;
      if (NIFCLOUD.util.Buffer.isBuffer(value) || value instanceof Stream) return;
    }

    var types = ['Buffer', 'Stream', 'File', 'Blob', 'ArrayBuffer', 'DataView'];
    if (value) {
      for (var i = 0; i < types.length; i++) {
        if (NIFCLOUD.util.isType(value, types[i])) return;
        if (NIFCLOUD.util.typeName(value.constructor) === types[i]) return;
      }
    }
