...
/**
 * A set of utility methods for use with the NIFCLOUD SDK.
 *
 * @!attribute abort
 *   Return this value from an iterator function {each} or {arrayEach}
 *   to break out of the iteration.
 *   @example Breaking out of an iterator function
 *     NIFCLOUD.util.each({a: 1, b: 2, c: 3}, function(key, value) {
 *       if (key == 'b') return NIFCLOUD.util.abort;
 *     });
 *   @see each
 *   @see arrayEach
 * @api private
...
var util = {
  ...
  userAgent: function userAgent() {
    ...
    var agent = 'nifcloud-sdk-' + name + '/' + require('./core').VERSION;
  ...
  uriEscape: function uriEscape(string) {
    ...
    // NIFCLOUD percent-encodes some extra non-standard characters in a URI
  ...
  date: {
     ...
     *   NIFCLOUD services rely on this date object, you can override
     *   this function to provide a special time value to NIFCLOUD service
     *   requests.
     */
    getDate: function getDate() {
      if (!NIFCLOUD) NIFCLOUD = require('./core');
      if (NIFCLOUD.config.systemClockOffset) { // use offset when non-zero
        return new Date(new Date().getTime() + NIFCLOUD.config.systemClockOffset);
      } else {
        return new Date();
      }
    },
